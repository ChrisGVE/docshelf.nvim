...
test("an alias names the reference too", function()
  ...
  eq(languages.kind("postgresql~17", "SQL"), "language")
  eq(languages.kind("mariadb", "SQL"), "package")
end)
...
test("user aliases add to the shipped ones", function()
  languages.configure({ languages = { add = { { "python", aliases = { "cpython" } } } } })
  eq(languages.find("py"), "Python")
  eq(languages.find("cpython"), "Python")
  reset()
end)
...
test("only entries in the user's list are guessed", function()
  ...
  languages.configure({ languages = { add = { "Haskell" } } })
