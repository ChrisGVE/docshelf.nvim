...
- `ApidocsSearch` (requires telescope.nvim or snacks.nvim) - open a picker to grep for text in all apidocs. If you want to display only a subset of sources, call the lua function: `:lua require("apidocs").apidocs_search({restrict_sources={"rust"}})`. The `layout` option works here too. Each match is shown once: the install writes a `.rgignore` in each source folder listing the per-entry section files, which repeat text from their page (the open picker still lists them), and matches inside a page's footer of links are left out. Sources installed before this change show repeats until they are reinstalled.
  ...
  Sources know their language, so a filter can widen itself: `require("apidocs.languages").pulled_in(selected, installed)` returns the installed packages of every selected language (selecting `python~3.14` pulls in `numpy~2.5` and `scikit_learn`; selecting `numpy~2.5` alone pulls in nothing). A source's language is decided once, at install, and kept in `.installed.json`: a source that declares its language (a Hackage package is Haskell) uses it; a devdocs source found in `lua/apidocs/source_languages.lua` takes that table's answer (a hand-maintained table classifying every devdocs source as a language reference, a package of a language, or a tool with no language, such as `git`); otherwise a source whose whole name before `~` is a language from your list, or one of its GitHub Linguist aliases, is that language's reference (`python~3.15`, `cpp`). Anything else is Unknown: `require("apidocs").assign_language(slug)` links it to a language picked from your list, and `require("apidocs.languages").unknown(installed)` lists those still Unknown. A link is final; reinstall the source to change it. Your list is the `languages` setup option (see below). Language names are those of GitHub Linguist, all 835 of them, in `lua/apidocs/linguist_languages.lua` (generated by `scripts/gen_linguist_languages.py`). Versions are not linked. Because the table is kept by hand, `nvim --headless -l scripts/check_source_languages.lua` compares it with the live devdocs catalogue: it lists sources the table lacks (each with a proposed row seeded from its repository's GitHub language, to be checked by a person) and rows for sources devdocs no longer lists. It never edits the table, and it cannot tell that an existing row is wrong.
...
return {
  ...
  config = function()
    ...
    -- The languages a source can be linked to (GitHub Linguist names). The default list is Python, Rust, C, C++, Java,
    -- TypeScript, JavaScript, Go, Odin, Zig, SQL, Markdown, the JSON and YAML flavours, TOML and Typst.
    -- 'add' extends it, 'only' replaces it:
    -- require('apidocs').setup({languages = {add = {"Haskell", "Lua"}}})
  end,
  keys = {
